...
static std::vector<std::string> _split(std::string to_split,
...
void BLEController::beginBluetooth(void) {
  ...
  _controlCharacteristic = _service->createCharacteristic(
      CONTROL_CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ |
  ...
  _telemetryCharacteristic = _service->createCharacteristic(
      TELEM_CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ |
                                     BLECharacteristic::PROPERTY_NOTIFY |
                                     BLECharacteristic::PROPERTY_WRITE_NR);
  ...
  _armCharacteristic = _service->createCharacteristic(
      ARM_CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ |
                                   BLECharacteristic::PROPERTY_NOTIFY |
                                   BLECharacteristic::PROPERTY_WRITE_NR);
  ...
  _resetCharacteristic = _service->createCharacteristic(
      RESET_CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ |
                                     BLECharacteristic::PROPERTY_NOTIFY |
                                     BLECharacteristic::PROPERTY_WRITE_NR);
  ...
  _debugCharacteristic = _service->createCharacteristic(
      DEBUG_CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ |
                                     BLECharacteristic::PROPERTY_NOTIFY |
                                     BLECharacteristic::PROPERTY_WRITE_NR);
  ...
  // Set the MTU size
  esp_err_t status = esp_ble_gatt_set_local_mtu(
      180);  // Replace 500 with the MTU size you want
  if (status != ESP_OK) {
  ...
  BLECharacteristic *manufacturerCharacteristic =
      _service->createCharacteristic(DEVINFO_MANUFACTURER_UUID,
                                     BLECharacteristic::PROPERTY_READ);
  manufacturerCharacteristic->setValue(DEVICE_MANUFACTURER);
  BLECharacteristic *deviceNameCharacteristic = _service->createCharacteristic(
      DEVINFO_NAME_UUID, BLECharacteristic::PROPERTY_READ);
  deviceNameCharacteristic->setValue(DEVICE_NAME);
  BLECharacteristic *deviceMacAddressCharacteristic =
      _service->createCharacteristic(DEVINFO_SERIAL_UUID,
                                     BLECharacteristic::PROPERTY_READ);
...
void BLEController::onMtuChanged(BLEServer *pServer,
...
void BLEController::onWrite(BLECharacteristic *characteristic) {
  ...
  if (characteristic->getUUID().equals(_controlCharacteristic->getUUID())) {
    ...
    if (components[0] == "motordebug_enabled") {
      ...
      LOG_INFO_ASYNC_ON_MAIN("Changing motor debug status to %s",
                             motorDebugEnabled ? "enabled" : "disabled");
      ...
      double motorWriteValue =
          ((((double)motorValue) / 255.0f) * 1000.0f) + 1000.0f;
      LOG_INFO_ASYNC_ON_MAIN("Updating motor %i to %f", motorNumber,
                             motorWriteValue);
      if (_motorDebugUpdateHandler) {
        const motor_debug_update_t motorDebugUpdate = {
            .motorNum = motorNumber, .motorWriteValue = (float)motorWriteValue};
        if (_motorDebugUpdateHandler) {
      ...
      return;
                 ...
                 _calibrationCharacteristic->getUUID())) {
    std::string value = characteristic->getValue();
    std::vector<std::string> components = _split(value, "=");
    ...
    if (_calibrationUpdateHandler) {
      const calibration_update_t calibrationUpdate = {.axis = axis,
                                                      .calibrationValue = val};
      _calibrationUpdateHandler(calibrationUpdate);
    }
  } else if (characteristic->getUUID().equals(
                 _debugCharacteristic->getUUID())) {
    std::string val = characteristic->getValue();
    LOG_INFO_ASYNC_ON_MAIN("Received debug command: %s", val.c_str());
    debug_recording_update_t debugDataUpdate = {.recordDebugData = false,
                                                .sendDebugData = false};
    if (val == "request") {
...
void BLEController::uploadDebugData(uint8_t *data, size_t length) {
  ...
  while (currentByteIndex < length) {
    if (currentByteIndex + packetSize < length) {
      ...
      _debugCharacteristic->setValue(&data[currentByteIndex],
                                     length - currentByteIndex);
    }
    _debugCharacteristic->notify();
    delay(30);
...
void BLEController::setDebugDataUpdateHandler(
