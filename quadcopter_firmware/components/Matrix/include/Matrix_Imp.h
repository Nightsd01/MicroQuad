...
template <Numeric T, size_t... Dims>
auto Matrix<T, Dims...>::transpose() const
  requires(ndims == 2)
{
  constexpr size_t rows = dimensions[0];
  constexpr size_t cols = dimensions[1];
...
template <Numeric T, size_t... Dims>
Matrix<T, Dims...> Matrix<T, Dims...>::invert() const
  requires(ndims == 2 && nth_element<0, Dims...>::value == nth_element<1, Dims...>::value)
{
  static_assert(FloatingPoint<T>, "Matrix inversion is numerically stable primarily for floating-point types");
...
template <Numeric T, size_t... Dims>
Matrix<T, Dims...> Matrix<T, Dims...>::identity()
  requires(ndims == 2 && nth_element<0, Dims...>::value == nth_element<1, Dims...>::value)
{
  constexpr size_t N = dimensions[0];
  Matrix<T, N, N> I;
...
auto Matrix<T, Dims...>::slice(size_t startRow, size_t startCol) const -> Matrix<T, SubRows, SubCols>
  requires(ndims == 2)
{
...
template <Numeric T, size_t... Dims>
template <size_t SubRows, size_t SubCols>
void Matrix<T, Dims...>::setSlice(size_t row, size_t col, const Matrix<T, SubRows, SubCols>& sub)
  requires(ndims == 2)
{
...
template <Numeric T, size_t... Dims>
T Matrix<T, Dims...>::determinant() const
  requires(ndims == 2 && nth_element<0, Dims...>::value == nth_element<1, Dims...>::value)
{
  constexpr size_t N = dimensions[0];
...
template <Numeric T, size_t... Dims>
auto Matrix<T, Dims...>::_createSubmatrix(size_t skip_row, size_t skip_col) const
  requires(ndims == 2)
{
  constexpr size_t N = dimensions[0];
...
template <Numeric T, size_t... Dims>
auto operator+(const Matrix<T, Dims...>& A, const Matrix<T, Dims...>& B)
    -> AddExpr<MatrixExpr<T, Dims...>, MatrixExpr<T, Dims...>>
{
...
template <Numeric T, size_t... Dims>
auto operator-(const Matrix<T, Dims...>& A, const Matrix<T, Dims...>& B)
    -> SubExpr<MatrixExpr<T, Dims...>, MatrixExpr<T, Dims...>>
{
...
template <Numeric T, size_t... Dims>
auto operator%(const Matrix<T, Dims...>& lhs, const Matrix<T, Dims...>& rhs)
    -> MulExpr<MatrixExpr<T, Dims...>, MatrixExpr<T, Dims...>>
{
  return MulExpr<MatrixExpr<T, Dims...>, MatrixExpr<T, Dims...>>(
      MatrixExpr<T, Dims...>(lhs),
      MatrixExpr<T, Dims...>(rhs));
}

// Element-wise division
template <Numeric T, size_t... Dims>
auto operator/(const Matrix<T, Dims...>& lhs, const Matrix<T, Dims...>& rhs)
    -> DivExpr<MatrixExpr<T, Dims...>, MatrixExpr<T, Dims...>>
{
  return DivExpr<MatrixExpr<T, Dims...>, MatrixExpr<T, Dims...>>(
      MatrixExpr<T, Dims...>(lhs),
      MatrixExpr<T, Dims...>(rhs));
}
...
// Scalar division
template <Numeric T, size_t... Dims>
auto operator/(const Matrix<T, Dims...>& A, const T& B) -> ScalarDivExpr<MatrixExpr<T, Dims...>>
{
  return ScalarDivExpr<MatrixExpr<T, Dims...>>(MatrixExpr<T, Dims...>(A), B);
}

// Reverse scalar division (scalar / matrix)
template <Numeric T, size_t... Dims>
auto operator/(const T& A, const Matrix<T, Dims...>& B) -> ReverseScalarDivExpr<MatrixExpr<T, Dims...>>
{
  return ReverseScalarDivExpr<MatrixExpr<T, Dims...>>(A, MatrixExpr<T, Dims...>(B));
}
...
template <typename LHS, typename RHS>
auto operator/(const LHS& lhs, const RHS& rhs) -> DivExpr<LHS, RHS>
  requires requires {
    typename LHS::value_type;
    typename RHS::value_type;
    { LHS::total_size } -> std::convertible_to<size_t>;
...
// Mixed Matrix / Expression division operators
template <Numeric T, size_t... Dims, typename Expr>
auto operator/(const Matrix<T, Dims...>& A, const Expr& B) -> DivExpr<MatrixExpr<T, Dims...>, Expr>
  requires requires {
    typename Expr::value_type;
    { Expr::total_size } -> std::convertible_to<size_t>;
    { B.eval(size_t{}) } -> std::convertible_to<T>;
...
{
  return DivExpr<MatrixExpr<T, Dims...>, Expr>(MatrixExpr<T, Dims...>(A), B);
}

template <Numeric T, size_t... Dims, typename Expr>
auto operator/(const Expr& A, const Matrix<T, Dims...>& B) -> DivExpr<Expr, MatrixExpr<T, Dims...>>
  requires requires {
    typename Expr::value_type;
    { Expr::total_size } -> std::convertible_to<size_t>;
    { A.eval(size_t{}) } -> std::convertible_to<T>;
...
{
  return DivExpr<Expr, MatrixExpr<T, Dims...>>(A, MatrixExpr<T, Dims...>(B));
}
