...
public:
    ...
    value_type eval(size_t i) const {
        return Op::apply(lhs_.eval(i), rhs_.eval(i));
    }
...
public:
    ...
    value_type eval(size_t i) const {
        return Op::apply(operand_.eval(i));
    }
...
public:
    ...
    value_type eval(size_t i) const {
        return Op::apply(operand_.eval(i), scalar_);
    }
...
public:
    ...
    value_type eval(size_t i) const {
        return Op::apply(scalar_, operand_.eval(i));
    }
...
struct NegOp {
    template<typename T>
    static T apply(const T& a) { return -a; }
};
