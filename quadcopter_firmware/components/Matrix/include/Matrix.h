...
class Matrix
{
 ...
 public:
  ...
  Matrix() { std::memset(data, 0, sizeof(data)); }

  // Constructor (initialize all elements to a specific value)
  explicit Matrix(const T& value) { std::fill(std::begin(data), std::end(data), value); }

  // Initialize from C array (in row-major/lexicographic order)
  explicit Matrix(const T (&values)[total_size]) { std::copy(std::begin(values), std::end(values), std::begin(data)); }

  // Initialize from pointer (for backward compatibility)
  explicit Matrix(const T* values) { std::copy(values, values + total_size, std::begin(data)); }
    ...
    }
  {
    ...
    static_assert(
        std::is_convertible_v<typename Expr::value_type, T>,
        "Expression value type must be convertible to matrix element type");
  ...
  void zeros() { std::memset(data, 0, sizeof(data)); }
  ...
  Matrix<T, Dims...> invert() const
    requires(ndims == 2 && nth_element<0, Dims...>::value == nth_element<1, Dims...>::value);

  // Identity matrix for square 2D matrices
  static Matrix<T, Dims...> identity()
    requires(ndims == 2 && nth_element<0, Dims...>::value == nth_element<1, Dims...>::value);

  // Slicing operations - for 2D matrices only
  template <size_t SubRows, size_t SubCols>
  auto slice(size_t startRow, size_t startCol) const -> Matrix<T, SubRows, SubCols>
    requires(ndims == 2);

  template <size_t SubRows, size_t SubCols>
  void setSlice(size_t row, size_t col, const Matrix<T, SubRows, SubCols>& sub)
    requires(ndims == 2);
 ...
 private:
  ...
  auto _createSubmatrix(size_t skip_row, size_t skip_col) const
    requires(ndims == 2);
};
