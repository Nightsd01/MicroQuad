...
#define QMC5883LObserverId int

class QMC5883L
{
 public:
  QMC5883L(TwoWire *pWire = &Wire, uint8_t I2C_addr = 0x0C);
  /**
   * @fn begin
   * @brief Sensor init
  ...
  QMC5883LObserverId addObserver(std::function<void(mag_update_t)> updateHandler);

  // returns true if successfully able to remove
  bool removeObserver(QMC5883LObserverId observerId);

  void loopHandler(void);

  void calibrate(float durationSeconds, std::function<void(bool, xyz_vector_t)> completionHandler);

  void setCalibrationOffsets(xyz_vector_t offsets);
 ...
 private:
  std::map<QMC5883LObserverId, std::function<void(mag_update_t)>> _updateHandlers;
  ...
  eDataRate_t _dataRate;
  float _gainFactor;
  xyz_vector_t _offsets;
  std::vector<QMC5883LObserverId> _observersToRemove;
};
