...
// Forward declaration of os_msys function
extern "C" struct os_mbuf* os_msys_get_pkthdr(uint16_t dsize, uint16_t user_hdr_len);
...
{
  // Extract size from 4-byte little-endian header
  _expectedDataSize = _headerBuffer[0] | (_headerBuffer[1] << 8) | (_headerBuffer[2] << 16) | (_headerBuffer[3] << 24);
...
int BLEControllerLargeDataTransmissionHandler::l2capEventCallback(struct ble_l2cap_event* event, void* arg)
{
  ...
  switch (event->type) {
    case BLE_L2CAP_EVENT_COC_CONNECTED: {
      ...
      struct os_mbuf* sdu_rx = os_msys_get_pkthdr(0, 0);
      if (sdu_rx) {
    ...
    case BLE_L2CAP_EVENT_COC_TX_UNSTALLED:
      // We're not sending data, so this shouldn't happen
      LOG_VERBOSE("L2CAP TX unstalled");
      return 0;
