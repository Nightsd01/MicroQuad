...
static constexpr uint64_t _bluetoothDisconnectEmergencyTimeoutMillis = 2000;  // 2 seconds
  ...
  switch (response) {
    ...
    case CalibrationResponse::Continue: {
      ...
      _magCalibrator->startCalibration([&](bool success) {
        ...
        LOG_INFO(
            "Successfully calibrated compass with hard iron matrix: %s, "
            "soft iron matrix: %s",
...
static void _beginMagneticMotorInterferenceCalibration(void)
{
  if (!_armed) {
    LOG_ERROR(
        "Cannot start magnetometer-motors-compensation calibration when "
        "not armed");
  ...
  _motorMagCompensationHandler->beginCalibration(
      ...
      [&](bool success) {
        ...
        if (!success) {
          LOG_ERROR(
              "Failed to complete magnetometer motors compensation "
              "calibration");
          _bluetoothController.sendCalibrationUpdate(
              CalibrationType::MagnetometerMotorsCompensation,
              CalibrationRequest::Failed);
...
void loop()
{
  ...
  if (_bluetoothController.isConnected) {
    _lastBluetoothConnectedTimeMillis = millis();
      ...
      _armed && !_enteredEmergencyMode &&
      (millis() - _lastBluetoothConnectedTimeMillis) > _bluetoothDisconnectEmergencyTimeoutMillis) {
