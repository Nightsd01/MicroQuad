...
PIDController::PIDController(gains_t gains, DebugHelper *helper) {
  _gains = gains;
  _helper = helper;
  _firstRun = true;
}

double PIDController::computeOutput(double current, double set,
                                    double timeSeconds) {
  const double deltaTimeSeconds =
      _firstRun ? 0.00001f : timeSeconds - _previousTimeSeconds;
  _firstRun = false;
  ...
  return (_gains.kP * error) + (_gains.kI * _integral) +
