...
static inline FusionVector Feedback(const FusionVector sensor,
                                    const FusionVector reference);
...
void FusionAhrsSetSettings(FusionAhrs *const ahrs,
  ...
  ahrs->settings.accelerationRejection =
      settings->accelerationRejection == 0.0f
          ? FLT_MAX
          : powf(0.5f * sinf(FusionDegreesToRadians(
  ...
  ahrs->settings.magneticRejection =
      settings->magneticRejection == 0.0f
          ? FLT_MAX
          : powf(0.5f *
                     sinf(FusionDegreesToRadians(settings->magneticRejection)),
                 2);
...
#define Q ahrs->quaternion.element
  ...
  if (FusionVectorIsZero(magnetometer) == false) {
    ...
    ahrs->halfMagnetometerFeedback =
        Feedback(FusionVectorNormalise(
...
static inline FusionVector Feedback(const FusionVector sensor,
                                    const FusionVector reference) {
  if (FusionVectorDotProduct(sensor, reference) <
 ...
 */
FusionAhrsInternalStates FusionAhrsGetInternalStates(
